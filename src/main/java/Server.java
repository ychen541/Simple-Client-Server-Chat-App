				...
				try {
					...
					if (type.equals("private chat request")) {
						...
						if (recipient == count) {
							...
							updateClients("You are talking to yourself :)", count);
						}
						else {
							updateClients("[PRIVATE]click to reply"+ ": " + data +" --from client #" + count , recipient);
						}
					} else if(type.equals("check request")){
						updateClients("-!want a group chat! click to select clients! --" , count);
						for(int i=0; i <clientList.size(); i++){
							updateClients("(Available): client #" + clientList.get(i), count);
						}

					}
					else if(type.equals("group")){
						String groupdata = in.readObject().toString();
						//int recipient = in.readInt();
						int recipient = in.readInt();
						if (recipient==count) {
							updateClients("[GROUP]"+ ": " + groupdata +" --from client myself" + count , count);
						}
						else {
							updateClients("[GROUP]" + ": " + groupdata + " --from client #" + count, recipient);
